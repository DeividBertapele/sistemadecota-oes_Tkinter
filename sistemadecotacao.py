...
def selecionar_arquivo():
    cam_arq = askopenfilename(title="Selecione o Arquivo de Moeda")
    var_cam_arq.set(cam_arq)
    if cam_arq:
        label_arquivoselecionado['text'] = f"Arquivo Selecionado: {cam_arq}"


def atualizar_cotacoes():
    try:
        # ler o dataframe de moedas
        df = pd.read_excel(var_cam_arq.get()) 
        moedas = df.iloc[:, 0]
        # pegar a data de inicio e data de fim das cotacoes
        data_inicial = calendar_data_inicial.get() # Inicial
        ano_inicial = data_inicial[-4:]
        mes_inicial = data_inicial[3:5]
        dia_inicial = data_inicial[:2]
        
        data_final = calendar_data_final.get() # Final
...
calendar_data_inicial = DateEntry(year=2022, locale='pt_br')
calendar_data_final = DateEntry(year=2022, locale='pt_br')
calendar_data_inicial.grid(row=7, column=1, padx=10, pady=10,  sticky='nsew')
calendar_data_final.grid(row=8, column=1, padx=10, pady=10, sticky='nsew')
